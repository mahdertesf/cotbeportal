...
export default function TeacherProfilePage() {
  ...
  useEffect(() => {
    if (storeUser?.user_id) {
      const loadProfile = async () => {
        setIsLoading(true);
        try {
          const data = await fetchTeacherProfile(storeUser.user_id);
          setProfileData(data);
        } catch (error) {
  ...
  const handleSaveChanges = async () => {
    ...
    setIsSaving(true);
    try {
      // Only send mutable fields for teachers
      const { office_location, phone_number, email } = profileData;
      const updatePayload: Partial<UserProfile> = { office_location, phone_number, email };
      
      const response = await updateTeacherProfile(storeUser.user_id, updatePayload);
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {renderSkeletonField("Office Location")}
