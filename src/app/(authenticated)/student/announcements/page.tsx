...
interface Announcement {
  ...
  content: string;
  publish_date: string;
  author_id: string; // Could be used to display author name if users are fetched
}
  ...
  useEffect(() => {
    if (user?.user_id && user.role === 'Student') {
      ...
      loadAnnouncements();
        ...
        setIsLoading(false); // Not a student, no need to load student announcements
    }
  }, [user, toast]);
      ...
      {announcements.length > 0 ? (
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-center text-muted-foreground">No announcements available for you at this time.</p>
          </CardContent>
        </Card>
      )}
    </div>
